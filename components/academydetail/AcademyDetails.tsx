...
const AcademyDetails: React.FC<ComponentProps> = ({ academy }) => {
  ...
  const [buttonText, setButtonText] = useState<string>(() => {
    const currentItem = cartItems.find((item) => item._id === academy._id)
    return currentItem ? 'Remove from Cart' : 'Add to Cart'
  })

  useEffect(() => {
    const currentItem = cartItems.find((item) => item._id === academy._id)
    setButtonText(currentItem ? 'Remove from Cart' : 'Add to Cart')
  }, [academy._id, cartItems])
  ...
  const handleAddToCart = () => {
    const cartItem = cartItems.find((item) => item._id === academy._id)
    if (cartItem) {
      const updatedItems = cartItems.filter((item) => item._id !== academy._id)
      ...
      const updatedItems = [...cartItems, academy]
      dispatch(setCartItems(updatedItems))
      sessionStorage.setItem('sessionCartItems', JSON.stringify(updatedItems))
